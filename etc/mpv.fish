...
function _complete_mpv_option -a opt
    mpv $opt=help | grep -v ':' | string trim \
...
function _complete_mpv_profile_option -a opt
    mpv $opt= | grep -v ':' | string trim
end

# Cache all the mpv options
set --local mpv_options (mpv --list-options | grep '^\s*--' | string trim)

for opt_line in $mpv_options
    set --local splitted (string split --max 1 ' ' -- $opt_line | string trim)
    set --local opt $splitted[1]
    set --local doc $splitted[2]
