...
static void deint_process(struct mp_filter *f)
{
    ...
    if (img->imgfmt == p->prev_imgfmt && p->deinterlace_active == (opts->deinterlace != 0)) {
        mp_subfilter_continue(&p->sub);
        return;
    }
    ...
    p->deinterlace_active = opts->deinterlace != 0;
    ...
    if (img->imgfmt == IMGFMT_VDPAU) {
        char *args[] = {"deint", "yes",
                        "interlaced-only", opts->deinterlace ? "no" : "yes",
...
bool mp_deint_active(struct mp_filter *f)
{
    struct deint_priv *p = f->priv;
    struct filter_opts *opts = p->opts->opts;
    return opts->deinterlace || (opts->deinterlace == -1 && p->interlaced_frame);
}
